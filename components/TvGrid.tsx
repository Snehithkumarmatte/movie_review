...
export function TvGrid({ title, items, emptyMessage }: { title: string, items: any[], emptyMessage?: string }) {
  return (
    <section className="space-y-3">
      ...
      {items.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {items.map((m:any) => (
            <Link key={m.id} href={`/tv/${m.id}`} className="no-underline group">
              ...
              <div className="mt-2">
                ...
                <div className="text-xs text-zinc-500">{m.first_air_date?.slice(0,4) ?? '—'}</div>
              </div>
            </Link>
          ))}
        </div>
