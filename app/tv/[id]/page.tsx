...
export async function generateMetadata({ params }: { params: { id: string }}) {
  ...
  return {
    title: `${data.name} — Reviews & Details`,
    ...
    <div className="grid md:grid-cols-[220px,1fr] gap-6">
      <div className="space-y-3">
        ...
        <div className="text-sm text-zinc-500 space-y-1">
          ...
          <div>First Air: {formatDate(data.first_air_date)}</div>
          <div>Seasons: {data.number_of_seasons}</div>
          ...
          <div className="grid gap-3">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {data.recommendations.results.slice(0,8).map((m:any) => (
                <Link key={m.id} href={`/tv/${m.id}`} className="rounded-2xl p-3 bg-zinc-100 dark:bg-zinc-900/50">
                  <div className="font-medium">{m.name}</div>
                  <div className="text-xs text-zinc-500">{new Date(m.first_air_date).getFullYear()}</div>
                </Link>
              ))}
            </div>
          </div>
